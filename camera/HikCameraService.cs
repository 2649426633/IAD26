...
namespace _180Detection.Camera
{
    ...
    {
        private readonly string _configuredSdkAssembly;
        ...
        public HikCameraService(string configuredSdkAssembly = null)
        {
            _configuredSdkAssembly = string.IsNullOrWhiteSpace(configuredSdkAssembly)
                ? string.Empty
                : Environment.ExpandEnvironmentVariables(configuredSdkAssembly.Trim());
        }

        public bool IsSdkAvailable { get { return EnsureSdkLoaded(); } }
        public bool IsConnected { get { return _connectedDevice != null; } }
        public string ConnectedCameraName { get { return _connectedName ?? string.Empty; } }
        ...
        public IList<HikCameraDevice> RefreshDevices()
        {
            ...
            try
            {
                ...
                result = parameters.Length == 0
                    ? enumMethod.Invoke(null, null)
                    : enumMethod.Invoke(null, new object[] { BuildDeviceLayerFilter(parameters[0].ParameterType) });
            }
        ...
        public void Connect(int deviceIndex)
        {
            ...
            if (deviceIndex < 0 || deviceIndex >= _devices.Count)
                throw new ArgumentOutOfRangeException(nameof(deviceIndex), "请选择一个可用的海康相机。");
        ...
        public void Disconnect()
        {
            ...
            try
            {
                MethodInfo close = device.GetType().GetMethod(
                    "Close", BindingFlags.Public | BindingFlags.Instance,
        ...
        private bool EnsureSdkLoaded()
        {
            ...
            if (!string.IsNullOrWhiteSpace(_configuredSdkAssembly) &&
                TryLoadAssemblyFrom(_configuredSdkAssembly))
                return true;
            ...
            foreach (string name in new[] { "MvCameraControl.Net", "MvCameraControl" })
            {
                try
                {
            ...
            _sdkLoadError = "未找到海康 MVS .NET SDK。请安装 MVS，或在“系统设置”配置 MVS .NET SDK DLL。";
            return false;
        }
        ...
        private bool TryFindAndLoadFromMvsDirectory(string root)
        {
            ...
            try
            {
                foreach (string file in Directory.GetFiles(root, "MvCameraControl*.dll", SearchOption.AllDirectories))
                {
        ...
        private static string BuildDisplayName(object info, int index)
        {
            ...
            string text = !string.IsNullOrWhiteSpace(userName)
                ? userName.Trim()
            ...
            if (!string.IsNullOrWhiteSpace(serial) &&
                text.IndexOf(serial, StringComparison.OrdinalIgnoreCase) < 0)
                text += "  [" + serial.Trim() + "]";
            return text;
        }
